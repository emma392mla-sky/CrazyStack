...
// SINGLE CLIENT - Project B has BOTH users AND payments tables
...
//=============================================
// MAIN APPLICATION CLASS - UNIFIED PROJECT B
//=============================================
class CrazyStackApp {
    constructor() {
    ...
    async forceRefreshBalance(source = 'unknown', retryCount = 0) {
        ...
        try {
            ...
            // Query Project B's users table directly
            ...
            console.log(`📊 [${source}] Project B DB Response:`, { data: dbBalance, error });
            ...
            console.log(`✅ [${source}] Balance fetched from Project B: MWK ${newBalance.toFixed(2)}`);
    ...
    async restoreUserSession(hood) {
        ...
        try {
            ...
            console.log('💰 Fetching FRESH balance from Project B...');
            ...
            console.log('✅ Session restored successfully from Project B!');
    ...
    startSessionMonitoring() {
        ...
        if(this.isUserAuthenticated && this.playerPhone && this.sessionToken) {
            ...
            this.sessionMonitorInterval = setInterval(async () => {
                try {
                    // Check session against Project B's users table
    ...
    forceLogoutDueToTimeout() {
        ...
        if(this.playerPhone && this.sessionToken) {
            // Clear token in Project B's users table
            supabaseClient.from('users')
                ...
                .then(({error}) => {
                    ...
                    else console.log('✅ Server token cleared in Project B');
                })
    ...
    async updateDatabaseBalance(amount, phone) {
        try {
            ...
            console.log(`📊 Updating Project B balance: ${nA > 0 ? '+' : ''}${nA} for ${phone}`);
            
            // Update Project B's users table directly
            ...
            console.log(`✅ Project B DB updated. New balance: MWK ${newBalance.toFixed(2)}`);
        ...
        if (isWin) {
            ...
            this.updateDatabaseBalance(pA, this.playerPhone)
                .then(async (nB) => {
                    ...
                    // Save bet to Project B's bets_history table
                ...
                .then(({ data, error }) => {
                    ...
                    else console.log('✅ Bet saved to Project B:', data);
                })
                .catch(err => {
    ...
    async processLogin() {
        ...
        try {
            ...
            console.log('🔐 ATTEMPTING LOGIN (Project B)');
            console.log('Raw Input:', pI);
            console.log('Normalized:', localPhone);

            document.getElementById('paymentPhoneInput').value=pI;
            
            // Login against Project B's users table
            ...
            if (rpcError || !userData || userData.length === 0) {
                ...
                console.log('✅ Login successful on Project B!');
                ...
                this.isUserAuthenticated = true;
                this.playerName = user.name || localPhone;
                ...
                setTimeout(async () => {
                    console.log('🔄 Verifying login balance with Project B...');
    ...
    // =============================================
    // SIGNUP - WITH PHONE NORMALIZATION
    // =============================================
    async processSignup() {
        ...
        try {
            ...
            console.log('📝 Creating account on Project B...');
            console.log('Raw Input:', pI);
            console.log('Normalized:', localPhone);
            
            // Create user in Project B's users table
            ...
            if (rpcError) {
                ...
                console.log('✅ Account created on Project B!');
    ...
    async processPaymentTransaction(event) {
        ...
        try {
            ...
            // ── IMPROVED POLLING v2.0 ──
            // Uses DUAL CHECKING:
            // 1. Primary: Check payments table (standard)
            // 2. Fallback: Check if balance increased (catches edge function errors)
            ...
            const pI = setInterval(async()=>{
                ...
                try{
                    ...
                    if (!pRec || pRec.status !== 'success') {
                        // No success in payments yet, check if balance changed!
                        await app.forceRefreshBalance('polling-' + pA);
                        
                        const currentBal = app.currentBalance;
                        const expectedNewBal = balanceBefore + tA;
                        const balanceDiff = Math.abs(currentBal - expectedNewBal);
                        
                        console.log(`🔍 [${pA}/${mP}] No payment record yet`);
                        console.log(`   Balance: ${currentBal} | Expected: ${expectedNewBal} | Diff: ${balanceDiff}`);
                        
                        // If balance increased by ~95%+, consider it success!
                        if (currentBal >= (balanceBefore + (tA * 0.95))) {
                            console.log(`✅✅✅ SUCCESS DETECTED BY BALANCE INCREASE!`);
                            foundSuccessByBalance = true;
                            
                            // Mark as processed so we don't poll again
                    ...
                    if ((pRec && pRec.status === 'success') || foundSuccessByBalance) {
                        ...
                        console.log('\n✅ PAYMENT CONFIRMED!');
                        console.log(`   Source: ${foundSuccessByBalance ? 'BALANCE_CHECK' : 'PAYMENTS_TABLE'}`);
                        ...
                        try {
                            ...
                            // Close modal faster (2s instead of 3s)
                            setTimeout(() => {
                            ...
                            // Still show success to user (payment WAS processed!)
                            showToastMessage(
                        ...
                        // ✅ FIXED: This is now OUTSIDE the try-catch block!
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('🎮 INITIALIZING CRAZYSTACK (Unified Project B)');
    
    window.crazyStackApp = new CrazyStackApp();
    
    console.log('✅ Application initialized - Connected to Project B only');
